...
fn main() -> Result<()> {
    ...
    let tx = program
        .request()
        .accounts(gfx_swap::accounts::Deposit1 {
            pool: pool,
            ...
            in_token_ata_user: get_associated_token_address(&user.pubkey(), &opt.token),
        ...
        .args(gfx_swap::instruction::Deposit1 {
            in_token_amount: token_mint.decimals.apply(opt.amount),
            minimum_pool_token_amount: LP_TOKEN_DECIMALS.apply(0),
        })
        .signer(&user)
        .send()
